import path__default, { resolve, dirname } from 'path';
import { isEmptyStatement, isClassDeclaration, isFunctionDeclaration, isInterfaceDeclaration, isModuleDeclaration, isIdentifier, isEnumDeclaration, isVariableStatement, isCallExpression, isObjectLiteralExpression, isPropertyAssignment, sys, ScriptTarget, createCompilerHost, createProgram as createProgram$1, findConfigFile, readConfigFile, formatDiagnostic, parseJsonConfigFileContent, formatDiagnostics, isLiteralExpression, isPropertyAccessExpression, isPrivateIdentifier, getCombinedModifierFlags, idText, SyntaxKind, isComputedPropertyName, isPropertyDeclaration, isPropertySignature, isIndexSignatureDeclaration, isMethodDeclaration, isMethodSignature, isConstructorDeclaration, isConstructSignatureDeclaration, isCallSignatureDeclaration, isGetAccessorDeclaration, isSetAccessorDeclaration, isTypeReferenceNode, isTypeLiteralNode, isArrayTypeNode, isTupleTypeNode, isParenthesizedTypeNode, isTypeOperatorNode, isTypePredicateNode, isUnionTypeNode, isIntersectionTypeNode, isMappedTypeNode, isConditionalTypeNode, isIndexedAccessTypeNode, isFunctionOrConstructorTypeNode, isImportTypeNode, isTypeQueryNode, isTypeNode, isInferTypeNode, isLiteralTypeNode, isStringLiteral, isModuleBlock, isTypeAliasDeclaration, isExportDeclaration, isNamespaceExport, ModifierFlags, isExportAssignment, isImportDeclaration, NodeFlags, isNamedImports, createSourceFile, DiagnosticCategory, nodeModuleNameResolver } from 'typescript';
        ...
        for (const node of this.sourceFile.statements) {
            ...
            for (const prop of obj.properties) {
                if (!isPropertyAssignment(prop) ||
                    ...
                    (prop.name.text !== "__proto__" && !isIdentifier(prop.initializer))) {
...
function getCompilerOptions(input) {
    ...
    let dtsFiles = [];
    const configPath = findConfigFile(dirname(input), sys.fileExists);
    if (!configPath) {
        return { dtsFiles, dirName, compilerOptions: Object.assign({}, OPTIONS_OVERRIDE) };
    }
    dirName = dirname(configPath);
    const { config, error } = readConfigFile(configPath, sys.readFile);
    if (error) {
        console.error(formatDiagnostic(error, formatHost));
        return { dtsFiles, dirName, compilerOptions: Object.assign({}, OPTIONS_OVERRIDE) };
    }
    const { fileNames, options, errors } = parseJsonConfigFileContent(config, sys, dirName);
    dtsFiles = fileNames.filter(name => name.endsWith(dts));
    if (errors.length) {
        console.error(formatDiagnostics(errors, formatHost));
        return { dtsFiles, dirName, compilerOptions: Object.assign({}, OPTIONS_OVERRIDE) };
    }
    return {
        dtsFiles,
...
function createPrograms(input) {
    ...
    for (let main of input) {
        ...
        if (options.dirName === dirName) {
            ...
            const program = createProgram$1(inputs.concat(Array.from(dtsFiles)), compilerOptions, host);
...
function matchesModifier(node, flags) {
    const nodeFlags = getCombinedModifierFlags(node);
...
class DeclarationScope {
    ...
    fixModifiers(node) {
        ...
        for (const mod of node.modifiers) {
            if (mod.kind !== SyntaxKind.ExportKeyword && mod.kind !== SyntaxKind.DefaultKeyword) {
                modifiers.push(mod.getText());
            }
            if (mod.kind === SyntaxKind.DeclareKeyword) {
                hasDeclare = true;
            }
        ...
        if (!hasDeclare && (isClassDeclaration(node) || isFunctionDeclaration(node))) {
    ...
    convertPropertyAccess(node) {
        ...
        let object = isIdentifier(node.expression)
    ...
    convertTypeNode(node) {
        ...
        if (isParenthesizedTypeNode(node) || isTypeOperatorNode(node) || isTypePredicateNode(node)) {
        ...
        if (isFunctionOrConstructorTypeNode(node)) {
            this.convertParametersAndType(node);
            return;
        }
        ...
        if (isTypeNode(node) && node.kind === SyntaxKind.RestType) {
            this.convertTypeNode(node.type);
            return;
        }
        if (isTypeNode(node) && node.kind === SyntaxKind.OptionalType) {
            this.convertTypeNode(node.type);
            return;
        }
        // istanbul ignore else
    ...
    convertImportTypeNode(node) {
        ...
        if (node.qualifier && isIdentifier(node.qualifier)) {
    ...
    convertNamespace(node) {
        ...
        for (const stmt of statements) {
            ...
            if (isInterfaceDeclaration(stmt) || isClassDeclaration(stmt)) {
            ...
            if (isExportDeclaration(stmt)) {
                if (stmt.exportClause) {
                    if (isNamespaceExport(stmt.exportClause)) {
...
class Transformer {
    ...
    maybeMarkAsExported(node, id) {
        ...
        if (!matchesModifier(node, ModifierFlags.Export) || !node.modifiers) {
            return false;
        }
        const isExportDefault = matchesModifier(node, ModifierFlags.ExportDefault);
    ...
    convertStatement(node) {
        ...
        if (node.kind == SyntaxKind.NamespaceExportDeclaration) {
    ...
    convertNamespaceDeclaration(node) {
        ...
        if (isGlobalAugmentation || !isIdentifier(node.name)) {
...
const plugin = (options = {}) => {
    ...
    function getModule(fileName) {
        ...
        // but only when there are zero ".ts" entry points.
            ...
            const code = sys.readFile(fileName, "utf8");
            if (code)
                source = createSourceFile(fileName, code, ScriptTarget.Latest, true);
        }
    ...
    return {
        ...
        load(id) {
            ...
            const { emitSkipped, diagnostics } = module.program.emit(module.source, (_, declarationText) => {
                const source = createSourceFile(declarationId, declarationText, ScriptTarget.Latest, true);
        ...
        resolveId(source, importer) {
            ...
            const { resolvedModule } = nodeModuleNameResolver(source, importer, compilerOptions, sys);
            if (!resolvedModule) {
                return;
            }
            if (!respectExternal && resolvedModule.isExternalLibraryImport) {
                ...
                return { id: path__default.resolve(resolvedModule.resolvedFileName) };
            }
        },
        renderChunk(code, chunk) {
            code = code.replace(/✂(\d+)\s*✂/g, (_, num) => fixups[num]);
            const source = createSourceFile(chunk.fileName, code, ScriptTarget.Latest, true);
